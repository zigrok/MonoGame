...
export EM_CACHE="${EM_CACHE:-$PACKS/Microsoft.NET.Runtime.Emscripten.$DOTNET_EMSCRIPTEN_VERSION.Cache.$EMSDK_HOST/$DOTNET_WASM_PACK_VERSION/tools/emscripten/cache}"
export PATH="$EMSDK_PATH/emscripten:$EMSDK_PATH/bin:$PATH"
export TMPDIR="${MONOGAME_BROWSER_SCRATCH:-$MONOGAME_ROOT/Artifacts/browser/scratch}"
mkdir -p "$TMPDIR"
